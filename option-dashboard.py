...
from py_vollib.black_scholes.greeks.analytical import delta, rho, vega, gamma, theta
from datetime import datetime, timedelta

st.title('Option Strategy Builder')

# Get number of option legs
num_options = st.sidebar.number_input("Total number of strikes/legs:", min_value=1, max_value=5, step=1, value=2, format="%d")
...
for i in range(num_options):
    ...
    position = st.sidebar.number_input(f"Position (1 for sell, -1 for buy) for leg {i + 1}:", min_value=-1, max_value=1, step=1, value=0, format="%d", key=f"position_{i}")
